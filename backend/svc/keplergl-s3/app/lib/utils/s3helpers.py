...
def getKeysStartingWithPrefixToNextDelimiter(s3client, s3_bucket, key_prefix, target_token_index_in_key, include_contents=True, include_common_prefixes=True):
    ret = []
    ...
    while resp is not None:
        ...
        if 'IsTruncated' in resp and resp['IsTruncated']:
            ...
            if 'NextMarker' in resp:
                ...
                print("Call to list_objects was truncated, but NextMarker not specified, so falling back to the last observed key per the documentation.  Perhaps try setting the delimiter?")
