...
def getPreviousMapVersions(s3client, mapId: str) -> List[int]:
    ...
    mapVersionKeys = getKeysStartingWithPrefixToNextDelimiter(
        s3client, KEPLERGL_S3_BUCKET, keyPrefix, 4, include_contents=False, include_common_prefixes=True
    )
...
def moveCurrentMapToArchive(s3client, mapId: str) -> int:
    ...
    datasetIds = getKeysStartingWithPrefixToNextDelimiter(
        s3client, KEPLERGL_S3_BUCKET, keyPrefix, 5, include_contents=False, include_common_prefixes=True
    )
...
def uploadMapDetailToS3(s3client, mapId: str, mapDetail: MapDetailResponse):
    ...
    try:
        ...
        s3client.upload_fileobj(mdh, KEPLERGL_S3_BUCKET, f"userzone/{KEPLERGL_S3_USER}/{mapId}/current/mapdetail.json")
    except ClientError as error:
        clientErrorCode = error.response["Error"]["Code"]
        raise CloudResourceError(f"S3 Client Error {clientErrorCode}")
...
def uploadDatasetToS3(s3client, mapId: str, datasetId: str, dataset: UploadFile):
    try:
        ...
        s3client.upload_fileobj(
            dataset.file, KEPLERGL_S3_BUCKET, f"userzone/{KEPLERGL_S3_USER}/{mapId}/current/datasets/{datasetId}/data.csv"
        )
    except ClientError as error:
        clientErrorCode = error.response["Error"]["Code"]
        raise CloudResourceError(f"S3 Client Error {clientErrorCode}")
...
def uploadColumnsJsonToS3(s3client, mapId: str, datasetId: str, columns: UploadFile):
    try:
        ...
        s3client.upload_fileobj(
            columns.file, KEPLERGL_S3_BUCKET, f"userzone/{KEPLERGL_S3_USER}/{mapId}/current/datasets/{datasetId}/columns.json"
        )
    except ClientError as error:
        clientErrorCode = error.response["Error"]["Code"]
        raise CloudResourceError(f"S3 Client Error {clientErrorCode}")
...
def getMapDetailFromS3(s3client, mapId: str) -> MapDetailResponse:
    try:
        mdh = BytesIO()
        s3client.download_fileobj(KEPLERGL_S3_BUCKET, f"userzone/{KEPLERGL_S3_USER}/{mapId}/current/mapdetail.json", mdh)
        return MapDetailResponse.parse_raw(mdh.getvalue().decode(encoding="utf-8"))
    except ClientError as error:
        clientErrorCode = error.response["Error"]["Code"]
        if clientErrorCode == "NoSuchKey":
            raise NotFoundError("S3 No Such Key Error")
        else:
            raise CloudResourceError(f"S3 Client Error {clientErrorCode}")
...
def getMapDetailsListFromS3(s3client) -> List[MapDetailResponse]:
    try:
        ...
        mapIds = getKeysStartingWithPrefixToNextDelimiter(
            s3client, KEPLERGL_S3_BUCKET, keyPrefix, 2, include_contents=False, include_common_prefixes=True
        )
...
def getDatasetFromS3(s3client, mapId: str, datasetId: str) -> StreamingBody:
    try:
        result = s3client.get_object(
            Bucket=KEPLERGL_S3_BUCKET, Key=f"userzone/{KEPLERGL_S3_USER}/{mapId}/current/datasets/{datasetId}/data.csv"
        )
        return result["Body"]
    except ClientError as error:
        clientErrorCode = error.response["Error"]["Code"]
...
def getColumnsFromS3(s3client, mapId: str, datasetId: str) -> StreamingBody:
    try:
        result = s3client.get_object(
            Bucket=KEPLERGL_S3_BUCKET, Key=f"userzone/{KEPLERGL_S3_USER}/{mapId}/current/datasets/{datasetId}/columns.json"
        )
        return result["Body"]
    except ClientError as error:
        clientErrorCode = error.response["Error"]["Code"]
...
def softDeleteMapFromS3(s3client, mapId: str) -> MapDetailResponse:
    ...
    deleteS3Object(s3client, f"userzone/{KEPLERGL_S3_USER}/{mapId}/current/mapdetail.json")
